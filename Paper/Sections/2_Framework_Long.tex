...
\end{equation*}
In the following, we will use this identification implicitly whenever no confusion can arise. Moreover, since $\varphi\in\cD(\Omega;X)$ has compact support, we have $\varphi\in L^2(\Omega;X)$ and therefore
\begin{equation*}
...
\end{equation*}
Thus, on test functions, the duality pairing of the regular distribution $T_f$ coincides with the $L^2$-inner product. In the Sobolev setting, we therefore use the terms distributional derivative and weak derivative interchangeably whenever the derivative is represented by an $L^2$-function.
...
\begin{definition}[Sobolev Spaces of Fractional Order]\label{def:fractionalsobolev}
	...
	\begin{equation*}
		...
		\right\},
	\end{equation*}
	where $\Diff^\alpha u$ is understood in the weak sense. Moreover, we define the fractional Sobolev semi-norm
	\begin{equation*}
		|u|_{s,\Omega}
		:=
		\left(
	...
	\end{equation*}
	and equip $H^s(\Omega;X)$ with the norm
	\begin{equation*}
		\|u\|_{s,\Omega}
		:=
		\left(
...
In variational formulations, boundary conditions are imposed on traces of bulk functions. For $u\in H^1(\Omega;X)$, pointwise boundary values are in general not defined. The trace theorem provides a canonical replacement. In this subsection, we assume that $\Omega\subset\R^d$ is a bounded Lipschitz domain and write $\Gamma:=\partial\Omega$. For Lipschitz boundaries, the outward unit normal $\mathrm{n}:\Gamma\to\R^d$ exists $\ds$-almost everywhere.

\begin{theorem}[Trace Theorem]\label{thm:trace}
	Let $X=\R^m$ with $m\ge 1$. There exists a unique linear continuous operator
	\begin{equation*}
		\gamma:H^1(\Omega;X)\to L^2(\Gamma;X)
	\end{equation*}
	which coincides with the classical restriction on $C^\infty(\overline{\Omega};X)$. Moreover, there exists $C_{\tr}>0$ such that
	\begin{equation*}
		\|\gamma u\|_{0,\Gamma}\le C_{\tr}\|u\|_{1,\Omega}
		\qquad\forall\,u\in H^1(\Omega;X).
	\end{equation*}

\end{theorem}

\begin{definition}[Trace Space]\label{def:HhalfGamma}
	We define the trace space of order $1/2$ by
	\begin{equation*}
		H^{1/2}(\Gamma;X):=\gamma\big(H^1(\Omega;X)\big)\subset L^2(\Gamma;X),
	\end{equation*}
	equipped with the quotient norm
	\begin{equation*}
		\|g\|_{1/2,\Gamma}
		:=
		\inf\left\{\|u\|_{1,\Omega}\;\middle|\;u\in H^1(\Omega;X),\ \gamma u=g\right\}.
	\end{equation*}
	Furthermore, we define
	\begin{equation*}
		H_0^1(\Omega;X):= \ker(\gamma).
	\end{equation*}
\end{definition}

With this definition, $\gamma:H^1(\Omega;X)\to H^{1/2}(\Gamma;X)$ is continuous and surjective.

\begin{definition}[Trace Spaces on Boundary Parts]\label{def:trace_parts}
	Let $\Gamma_0\subset\Gamma$ be relatively open. We define
	\begin{equation*}
		H^{1/2}(\Gamma_0;X)
		:=
		\left\{ g:\Gamma_0\to X\;\middle|\;\exists\,\widetilde g\in H^{1/2}(\Gamma;X)\text{ with }\widetilde g|_{\Gamma_0}=g\right\},
	\end{equation*}
	with norm
	\begin{equation*}
		\|g\|_{1/2,\Gamma_0}
		:=
		\inf\left\{\|\widetilde g\|_{1/2,\Gamma}\;\middle|\;\widetilde g\in H^{1/2}(\Gamma;X),\ \widetilde g|_{\Gamma_0}=g\right\}.
	\end{equation*}

	Moreover, we define
	\begin{equation*}
		H^{1/2}_{00}(\Gamma_0;X)
		:=
		\left\{ g\in H^{1/2}(\Gamma_0;X)\;\middle|\;\widetilde g\in H^{1/2}(\Gamma;X)\right\},
	\end{equation*}
	where $\widetilde g$ denotes the zero extension to $\Gamma$, i.e.\ $\widetilde g=g$ on $\Gamma_0$ and $\widetilde g=0$ on $\Gamma\setminus\Gamma_0$.
	We equip $H^{1/2}_{00}(\Gamma_0;X)$ with the norm
	\begin{equation*}
		\|g\|_{1/2,00,\Gamma_0}:=\|\widetilde g\|_{1/2,\Gamma}.
	\end{equation*}
\end{definition}
...
\subsection{$H(\diver;\Omega)$ and Normal Traces}

In elasticity, stress fields are (typically symmetric) matrix-valued functions. While $\tau\in L^2(\Omega;\mathrm{Sym}^2(\R^d))$ is sufficient to interpret volume integrals of the form
\begin{equation*}
	\int_\Omega \tau:\nabla v\;\dx,
\end{equation*}
...
\begin{proposition}[Integration by Parts for Smooth Fields]\label{pr:green_smooth}
...
\end{equation}
By Cauchy--Schwarz, $F_\tau$ is a continuous linear functional on $H^1(\Omega;\R^d)$.
...
\begin{definition}[Normal Trace / Boundary Traction]\label{def:normal_trace}
	...
	\end{equation}
\end{definition}
In the smooth case, \eqref{eq:greenHdiv} reduces to \eqref{eq:green_smooth}, hence $\gamma_{\mathrm{n}}(\tau)$ coincides with the classical traction $\tau\,\mathrm n$. We also define the equivalent concept on a boundary part $\Gamma_0\subset\Gamma$.
...
\begin{definition}[Normal Trace on a Boundary Part]\label{def:normal_trace_part}
	...
	\begin{equation*}
		...
		:=
		\langle \gamma_{\mathrm{n}}(\tau),\,\widetilde g\rangle_{\Gamma},
		\qquad g\in H^{1/2}_{00}(\Gamma_0;\R^d),
	\end{equation*}
	where $\widetilde g$ denotes the zero extension of $g$ from $\Gamma_0$ to $\Gamma$.
\end{definition}
