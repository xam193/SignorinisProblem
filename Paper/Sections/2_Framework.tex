...
\begin{definition}[Partial Derivative, Gradient, Jacobian and Divergence]
	...
	\begin{itemize}
		...
		\item[iv)] The {\tt divergence} of $F=(F_1,\ldots,F_m)^\top$ is
		      \begin{equation*}
			      \diver F := \sum_{j=1}^m \frac{\partial F_j}{\partial x_j}:\Omega\to\R.
		      \end{equation*}
	\end{itemize}
\end{definition}
...
\begin{remark}[Componentwise extension to matrix-valued fields]
	All definitions above extend componentwise to finite-dimensional target spaces. In particular, we identify
	\begin{equation*}
		\mathbb{R}^{m\times n}\cong \mathbb{R}^{mn},
	\end{equation*}
	and set
	\begin{equation*}
		\mathcal{D}'(\Omega;\mathbb{R}^{m\times n}) := (\mathcal{D}'(\Omega))^{m\times n},
		\qquad
		\partial^\alpha T := (\partial^\alpha T_{ij})_{i,j}.
	\end{equation*}
	For tensor fields $\tau=(\tau_{ij})_{i,j=1}^d$ we use the row-wise divergence
	\begin{equation*}
		\operatorname{div}\tau := \left(\sum_{j=1}^d \partial_j \tau_{ij}\right)_{i=1}^d
	\end{equation*}
	(in the classical or distributional sense, depending on regularity). Moreover, for $A,B\in\mathbb{R}^{d\times d}$ we write
	\begin{equation*}
