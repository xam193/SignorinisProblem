...
\begin{proposition}[Equivalence of mixed and KKT formulation]
	Assume $|\Gamma_N|=0$.
	If $u$ solves Problem \ref{pr:signorini5}, then the pair $(u,\sigma_{C, \mathrm n}(u))\in V\times \Lambda_C$ solves the mixed Problem \ref{pr:signorini7}. Conversely, if $(u,\lambda)\in V\times \Lambda_C$ solves the mixed Problem \ref{pr:signorini7}, then $u$ solves Problem \ref{pr:signorini5} and $\lambda=\sigma_{C, \mathrm n}(u)$.
\end{proposition}
\begin{proof}
	Assume first that $u$ solves Problem \ref{pr:signorini5}. Define $\lambda:=\sigma_{C, \mathrm n}(u)\in \Lambda_C$, where the inclusion in $\Lambda_C$ is exactly \eqref{eq:strong_lambda}. We show that $(u,\lambda)\in V\times \Lambda_C$ solves Problem \ref{pr:signorini7}. Since $u\in V$ and the bulk equation \eqref{eq:strong_bulk} holds with $f\in L^2(\Omega;\R^d)$, we have $\sigma(u)\in H(\diver;\Omega)$. Because $|\Gamma_N|=0$, the load functional reduces to
	\begin{equation*}
    ...
    \begin{equation}\label{eq:usefullocal}
        \langle \sigma_C(u)\mathrm n,\,\gamma_C v\rangle_{\Gamma_C} = a(u,v)-\ell(v)\qquad\forall\,v\in V.
    \end{equation}
    Using the decomposition $\sigma_C(u)\mathrm{n} = \sigma_{C, \mathrm n}(u)\mathrm{n}+\sigma_{C,t}(u)$, we further get
	\begin{equation*}
		\langle \sigma_C(u)\mathrm n,\,\gamma_C v\rangle_{\Gamma_C}
		=
		\langle \sigma_{C, \mathrm n}(u),\,v_{\mathrm n}\rangle_{\Gamma_C}
		+
		\langle \sigma_{C,t}(u),\,v_t\rangle_{\Gamma_C}.
	\end{equation*}
	By \eqref{eq:strong_tangential}, the tangential term vanishes, and since $\lambda=\sigma_{C, \mathrm n}(u)$
	we conclude that
	\begin{equation*}
	...
	\begin{equation*}
		...
		\qquad\text{in }W_{C,t}'.
	\end{equation*}
	On the other hand, because $|\Gamma_N|=0$ and \eqref{eq:strong_bulk} holds, the local Green formula \eqref{eq:usefullocal} gives
	\begin{equation*}
		a(u,v)-\ell(v)
		=
		\langle \sigma_C(u)\mathrm n,\,\gamma_C v\rangle_{\Gamma_C}
	...
	\begin{equation*}
		...
		\qquad\forall\,v\in V.
	\end{equation*}
	Using again the decomposition $\sigma_C(u)\mathrm{n} = \sigma_{C, \mathrm n}(u)\mathrm{n}+\sigma_{C,t}(u)$ and $\sigma_{C,t}(u)=0$ on $\Gamma_C$, this becomes
	\begin{equation*}
		\langle \sigma_{C, \mathrm n}(u),\,v_{\mathrm n}\rangle_{\Gamma_C}
		=
		\langle \lambda,\,v_{\mathrm n}\rangle_{\Gamma_C}
	...
	\end{equation*}
	and therefore $\lambda=\sigma_{C, \mathrm n}(u)$, which completes the proof.
\end{proof}